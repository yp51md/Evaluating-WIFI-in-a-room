...
def getRate(lightLocation):
    ...
    if fieldLocation[0] > 50 or fieldLocation[1] > 50:
        return 1
...
def isWall(lightLocation):
    ...
    #         field[0]):
    #     return -1  # boundary
    if fieldLocation[0] >= 51 or fieldLocation[1] >= 51:
        return -1
...
#    print(lights[i].strength)
#    for j in (0,50)
#    if (np.array(iLight.start) + j * np.array(iLight.direction))=[66][46]:
#        value=value+lights[i].strength
